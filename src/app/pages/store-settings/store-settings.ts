...
export class StoreSettings implements OnInit {
  ...
  StoreName = linkedSignal(()=> 'Mi tienda')
  ...
  newCostCategory = signal<'expense' | 'tax' | 'shipping'>('expense');
  newCostType = signal<
    'fixed' | 'percent_over_provider' | 'percent_over_price'
  >('fixed');
  ...
  emailTemplatesCatalog = [
    {
      ...
      icon: 'check_circle',
      desc: 'Enviado cuando el pago se aprueba y se confirma el pedido.',
      badge: 'Venta',
      color: 'text-success',
    },
    {
      ...
      desc: 'Instrucciones bancarias para que el cliente transfiera y suba comprobante.',
      badge: 'Pago',
      color: 'text-info',
    },
    {
      key: 'cashPayment',
      title: 'Pago en Efectivo / Local',
      icon: 'payments',
      desc: 'Instrucciones para abonar en el punto de retiro o contra entrega.',
      badge: 'Pago',
    ...
    },
    {
      ...
      icon: 'verified',
      desc: 'Notifica al cliente que su pago fue recibido y pasa a empaquetado.',
      badge: 'Estado',
      color: 'text-success',
    },
    {
      ...
      icon: 'hourglass_top',
      desc: 'Avisa que la pasarela de pagos está validando la transacción.',
      badge: 'Estado',
      color: 'text-warning',
    },
    {
      ...
      icon: 'local_shipping',
      desc: 'Notifica el envío con código de seguimiento y transporte.',
      badge: 'Envío',
      color: 'text-primary',
    },
    {
      ...
      icon: 'inventory_2',
      desc: 'Avisa la entrega y pide feedback / reseña al cliente.',
      badge: 'Entrega',
    ...
    },
    {
      ...
      desc: 'Recupera ventas recordando los productos dejados con un cupón.',
      badge: 'Marketing',
      color: 'text-error',
    },
    {
      ...
      icon: 'star',
      desc: 'Notifica a los usuarios con items en favoritos que volvió a haber stock.',
      badge: 'Marketing',
  ...
  defaultEmailTemplates: IEmailTemplatesConfig = {
    branding: {
      primaryColor: '#111827',
      footerText:
        '¡Gracias por comprar en nuestra tienda! Si tenés alguna duda, respondé directamente a este correo.',
    ...
    orderConfirmation: {
      ...
      message:
        'Recibimos tu pedido correctamente y ya lo estamos preparando para vos.',
      extraInstructions: '',
    ...
    bankTransfer: {
      ...
      message:
        'Hola {{cliente_nombre}}, tu pedido fue reservado. Realizá la transferencia bancaria con los siguientes datos y subí el comprobante para que podamos despacharlo.',
      extraInstructions:
        'Recordá que tenés 24hs para transferir y subir tu comprobante antes de que se libere el stock.',
    ...
    cashPayment: {
      ...
      message:
        'Tu pedido ya fue cargado en nuestro sistema para pago en efectivo al momento del retiro.',
      extraInstructions:
        'Podés abonar al momento de retirar en el local o coordinar con nuestro equipo.',
      buttonText: 'Ver Detalle del Pedido',
    },
    paymentReceived: {
      enabled: true,
      subject:
        '¡Pago acreditado! Tu pedido #{{numero_orden}} está listo para empaquetar 📦',
      heading: '¡Pago acreditado con éxito!',
      message:
        'Hola {{cliente_nombre}}, verificamos tu pago de {{total_orden}}. Tu orden ya pasó a preparación.',
      extraInstructions: '',
    ...
    paymentPending: {
      ...
      message:
        'Hola {{cliente_nombre}}, la pasarela de pagos está validando la transacción. Te avisaremos apenas se confirme.',
      extraInstructions: '',
    ...
    orderShipped: {
      ...
      message:
        'Hola {{cliente_nombre}}, tu paquete ya está en manos del correo o logística para la entrega.',
      extraInstructions:
        'Podés hacer el seguimiento de tu envío en tiempo real con el código provisto.',
    ...
    orderDelivered: {
      ...
      message:
        'Hola {{cliente_nombre}}, tu pedido figura como entregado. ¡Gracias por confiar en {{nombre_tienda}}!',
      extraInstructions:
        'Si te gustó tu producto, nos encantaría que nos dejes tu reseña o nos etiquetes en redes.',
    ...
    abandonedCart: {
      ...
      message:
        'Hola {{cliente_nombre}}, guardamos los productos que dejaste en tu carrito para que no te quedes sin stock.',
      extraInstructions: '',
    ...
    backInStock: {
      ...
      message:
        'Hola {{cliente_nombre}}, te avisamos que {{producto_nombre}} ya tiene stock disponible nuevamente.',
  ...
  constructor() {
    ...
    this.configForm = this.#fb.group({
      ...
      emailTemplates: this.#fb.group({
        ...
        orderConfirmation: this.createTemplateGroup(
          this.defaultEmailTemplates.orderConfirmation!,
        ),
        bankTransfer: this.createTemplateGroup(
          this.defaultEmailTemplates.bankTransfer!,
        ),
        cashPayment: this.createTemplateGroup(
          this.defaultEmailTemplates.cashPayment!,
        ),
        paymentReceived: this.createTemplateGroup(
          this.defaultEmailTemplates.paymentReceived!,
        ),
        paymentPending: this.createTemplateGroup(
          this.defaultEmailTemplates.paymentPending!,
        ),
        orderShipped: this.createTemplateGroup(
          this.defaultEmailTemplates.orderShipped!,
        ),
        orderDelivered: this.createTemplateGroup(
          this.defaultEmailTemplates.orderDelivered!,
        ),
        abandonedCart: this.createTemplateGroup(
          this.defaultEmailTemplates.abandonedCart!,
        ),
        backInStock: this.createTemplateGroup(
          this.defaultEmailTemplates.backInStock!,
        ),
      }),
    ...
    effect(() => {
      const { hasData, config, hasError, isLoading } =
        this.configState.StoreConfig();
      if (hasData && !hasError && !isLoading && config) {
        ...
        if (config.fiscalProfile) {
          this.configForm.get('fiscalProfile')?.patchValue({
            ...
            legalName:
              config.fiscalProfile.legalName ||
              config.fiscalProfile.businessName ||
              '',
            businessName:
              config.fiscalProfile.businessName ||
              config.fiscalProfile.legalName ||
              '',
            iibbNumber:
              config.fiscalProfile.grossIncomeNumber ||
              (config.fiscalProfile as any).iibbNumber ||
              '',
            monotributoCategory:
              config.fiscalProfile.monotributoCategory || 'C',
            previousExternalBilling:
              config.fiscalProfile.previousExternalBilling || 0,
          });
        }
        if (config.defaultAdditionalCosts) {
          this.configForm
            .get('defaultAdditionalCosts')
            ?.setValue(config.defaultAdditionalCosts);
        }
        if (config.integrations?.arca) {
          this.configForm
            .get('integrations.arca')
            ?.patchValue(config.integrations.arca);
        }
        if (config.shippingConfig) {
          this.configForm.get('shippingConfig')?.patchValue({
            freeShippingThreshold:
              config.shippingConfig.freeShippingThreshold ?? 80000,
            defaultItemSubsidy:
    ...
    effect(() => {
      ...
      if (success) {
        this.#NotificationService.success(
          'Mercado pago sincronizado con éxito',
        );
        this.cleanUrlParams();
      }
      ...
      if (error) {
        this.#NotificationService.error(
          'Error al sincronizar con Mercado Pago',
        );
        this.cleanUrlParams();
      }
    });
  ...
  copyApiKey() {
    ...
    if (key) {
      ...
      this.#NotificationService.success(
        '¡Llave de la tienda copiada al portapapeles! 📋',
      );
    }
  }
  ...
  async addDomain() {
    ...
    const currentOrigins =
      this.configState.connectionSettings()?.allowedOrigins || [];
  ...
  get currentTemplateControl(): FormGroup {
    return this.configForm.get(
      `emailTemplates.${this.selectedEmailTemplate()}`,
    ) as FormGroup;
  }
  ...
  get previewSender(): string {
    const tplFromName = this.currentTemplateControl
      ?.get('fromName')
      ?.value?.trim();
    const tplFromEmail = this.currentTemplateControl
      ?.get('fromEmail')
      ?.value?.trim();
    const globalFromName =
      this.configForm.get('integrations.resend.fromName')?.value?.trim() ||
      this.configForm.get('name')?.value ||
      'Mi Tienda';
    const globalFromEmail =
      this.configForm.get('integrations.resend.fromEmail')?.value?.trim() ||
      this.configForm.get('contact.email')?.value ||
      'ordenes@mitienda.com';
  ...
  lastFocusedField = signal<
    'subject' | 'heading' | 'message' | 'extraInstructions'
  >('message');

  get currentDefaultTemplate(): IEmailTemplateItem {
    return (
      (this.defaultEmailTemplates as any)[this.selectedEmailTemplate()] || {}
    );
  }

  setFocusedField(
    field: 'subject' | 'heading' | 'message' | 'extraInstructions',
  ) {
  ...
  resetTemplateToDefault(key: string) {
    ...
    if (defaultData) {
      ...
      this.#NotificationService.info(
        'Plantilla restaurada a los valores predeterminados',
      );
    }
  }
  ...
  async sendTestEmail() {
    ...
    if (!email || !email.includes('@')) {
      this.#NotificationService.error(
        'Por favor ingresá un email válido para la prueba',
      );
      return;
    }
    ...
    try {
      await this.#storeConfigService.sendTestEmail(
      ...
      this.#NotificationService.error(
        err?.error?.message ||
          'Error al enviar el email de prueba. Verificá tu API Key de Resend.',
      );
    } finally {
  ...
  get previewExtraInstructions(): string {
    const raw =
      this.currentTemplateControl?.get('extraInstructions')?.value || '';
    return this.renderMockVariables(raw);
  }

  get previewButtonText(): string {
    return (
      this.currentTemplateControl?.get('buttonText')?.value || 'Ver Detalle'
    );
  }
  ...
  get previewFooterText(): string {
    return (
      this.emailBrandingControl?.get('footerText')?.value ||
      `¡Gracias por comprar en ${this.configForm.get('name')?.value || 'nuestra tienda'}! Si tenés dudas, respondé directamente a este correo.`
    );
  }
  ...
  renderMockVariables(text: string): string {
    ...
    const mockVars: Record<string, string> = {
      ...
      transporte: 'Andreani',
      alias_transferencia:
        this.configForm.get('paymentGateways.transfer.alias')?.value ||
        'MI.TIENDA.PAGOS',
      cbu_transferencia:
        this.configForm.get('paymentGateways.transfer.cbuCvu')?.value ||
        '0000003100010002938472',
      banco_transferencia:
        this.configForm.get('paymentGateways.transfer.bankName')?.value ||
        'Banco Galicia',
      titular_transferencia:
        this.configForm.get('paymentGateways.transfer.titular')?.value ||
        'Martín Comercio S.A.',
      link_comprobante: '#',
  ...
  get activeSocialNetworks(): { name: string; url: string }[] {
    ...
    const wa =
      this.configForm.get('contact.whatsapp')?.value?.trim() ||
      this.configForm.get('contact.phone')?.value?.trim();
    ...
    if (tt)
      list.push({
        name: 'TikTok',
        url: `https://tiktok.com/@${tt.replace('@', '')}`,
      });
    if (wa)
      list.push({
        name: 'WhatsApp',
        url: `https://wa.me/${wa.replace(/[^0-9]/g, '')}`,
      });
  ...
  async saveFiscalSettings() {
    ...
    await this.saveSlice('fiscal', {
      ...
      defaultAdditionalCosts: this.configForm.get('defaultAdditionalCosts')
        ?.value,
      integrations: this.configForm.get('integrations')?.value,
    });
  }
  ...
  get currentTabSaveLabel(): string {
    switch (this.activeTab()) {
      case 'general':
        return 'Guardar Identidad & Logo';
      case 'auth':
        return 'Guardar Métodos de Autenticación';
      case 'fiscal':
        return 'Guardar Configuración Fiscal & ARCA';
      case 'integrations':
        return 'Guardar Integraciones';
      case 'emails':
        return 'Guardar Plantillas de Email';
      case 'contact':
  ...
  async saveConfig() {
    ...
    switch (tab) {
      ...
      case 'auth':
        return this.saveAuthSettings();
      case 'fiscal':
        return this.saveFiscalSettings();
      case 'integrations':
        return this.saveIntegrations();
      case 'emails':
        return this.saveEmailTemplates();
      case 'contact':
  ...
  addDefaultCost() {
    ...
    if (!concept) {
      this.#NotificationService.warning(
        'Ingresá el nombre o concepto del costo/impuesto',
      );
      return;
    }
    ...
    this.#NotificationService.success(
      `Costo por defecto "${concept}" agregado`,
    );
  }
  ...
  removeDefaultCost(index: number) {
    ...
    if (removed.length > 0) {
      this.#NotificationService.info(
        `Concepto "${removed[0].concept}" removido`,
      );
    }
  }
  ...
  async testArcaConnection() {
    ...
    try {
      ...
      this.#NotificationService.success(
        'Servidores de ARCA (AFIP) en línea y operativos',
      );
    } catch (err: any) {
      const msg =
        err?.error?.error || err?.message || 'Error de conexión con ARCA';
  ...
  async searchCuitInArca() {
    ...
    if (!rawCuit) {
      this.#NotificationService.error(
        'Ingresá un CUIT para buscar en el padrón de ARCA / AFIP',
      );
      return;
    }
    ...
    if (cleaned.length < 10 || cleaned.length > 11) {
      this.#NotificationService.error(
        'El CUIT debe contener 11 dígitos numéricos',
      );
      return;
    }
    ...
    try {
      const res: any = await firstValueFrom(
        this.#arcaService.getTaxpayer(cleaned),
      );
      ...
      if (taxpayer) {
        ...
        this.configForm.markAsDirty();
        const catInfo = taxpayer.monotributoCategory
          ? ` (Cat. ${taxpayer.monotributoCategory})`
          : '';
        this.#NotificationService.success(
          `Datos obtenidos de ARCA: ${taxpayer.legalName || cleaned}${catInfo}`,
        );
      } else {
        this.#NotificationService.info(
          'Consulta completada, pero no se recuperaron campos automáticos.',
        );
      }
    } catch (err: any) {
      const msg =
